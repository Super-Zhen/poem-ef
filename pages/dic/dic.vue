		...
		<header-bar titleTintColor="#000" :bgColor="{'background': '#000'}" >
			<view slot="headerL" style="width:100%;">
				...
				<view class="navbox">
					...
					<view class="width30">
						<view class="icon iconfont icon-tengxunweibo1" style="font-size: 30px;"
							v-show="currentIndex==='1'"></view>
					</view>
				</view>
			</view>
		...
		<view v-show="currentIndex === '2' && authorsList.length">
			<view class="border fontS14 mg15 pd20 borderR10" v-for="(item, index) in authorsList" :key="item.objectId">
				...
				<view>
					...
					<text class="colorf6495ed"
						@tap="toAuthorDetail(item.objectId,item.name)">{{item.worksCount}}篇诗文</text>
				</view>
			</view>
		...
		<view v-show="currentIndex=='3'">
			<view v-for="(item,index) in allworks" :key="index">
				<view class="mg15 border borderR10 pd10">
					...
					<view>
						<view v-for="(items,index) in item.content" :key="index"
							class="fontS15 textIndent2 textJustify mgb5 lineH28">
							{{items}}
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			async getHotAuthorsIncludeCountByLikers(data) {
				...
				} = await this.$api.works.getHotAuthorsIncludeCountByLikers(data)
			...
			async getAllWorksForH5(data) {
				let {
					result
				} = await this.$api.works.getAllWorksForH5(data)
